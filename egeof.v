...
Module FrenetSerret.
  Parameter (Fx Fy : R -> R). (* position x and y components *)

  Definition Tx := Derive Fx. (* x velocity component *)
  Definition Ty := Derive Fy. (* y velocity component *)

  Axiom is_derive_Fx : forall s, is_derive Fx s (Tx s).
  Axiom is_derive_Fy : forall s, is_derive Fy s (Ty s).

  (* The path (Fx s, Fy s) is parameterized by distance; velocity is 1 *)
  Axiom unitvelocity : forall s, (Tx s)² + (Ty s)² = 1.

  (* prove these? *)
  Axiom ex_derive_Tx : forall s, ex_derive Tx s.
  Axiom ex_derive_Ty : forall s, ex_derive Ty s.

  (* curvature is always defined -- no straight motion *)
  Axiom accel_nonzero : forall s, ((Derive Tx s)² + (Derive Ty s)²) <> 0.

  Definition κ s := sqrt ((Derive Tx s)² + (Derive Ty s)²).
  Definition Nx s := / κ s * Derive Tx s. (* accel direction x component *)
  Definition Ny s := / κ s * Derive Ty s. (* accel direction y component *)
  ...
  Lemma unitaccel : forall s, (Nx s)² + (Ny s)² = 1.
  Proof.
    intros.
    unfold Nx, Ny, κ.
    repeat rewrite Rsqr_mult.
  ...
  (* is_derive Tx s (Derive Tx s) *)
  Lemma is_derive_Tx_abort : forall s, is_derive Tx s 1.
  Proof.
    intros.
    rewrite Tx_def.
    set (f := fun v : R => cos (atan2 (Ty v) (Tx v))).
    set (g := fun v : R => Tx v / sqrt ((Tx v)² + (Ty v)²)).
    assert (locally s (fun t => g t = f t)) as feqg. {
      unfold locally.
      simpl.
      ...
      apply atan2_cos_id.
      specialize (unitvelocity u) as uv.
      apply nzm.
      rewrite uv.
      discrR. }
    simpl in feqg.
    ...
    eassumption.
    unfold g.
    specialize (unitvelocity s) as veq1.
    auto_derive.
    repeat (split; try (apply ex_derive_Tx)).
    apply ex_derive_Ty.
    rewrite veq1.
    lra.
    rewrite veq1.
...
(*
    Derive Tx s + Tx s * (- (Derive Tx s * Tx s + Derive Ty s * Ty s)) = Derive Tx s
->
Tx s * (Derive Tx s * Tx s + Derive Ty s * Ty s) = 0
->
Tx s = 0 \/ Derive Tx s = - Derive Ty s * Ty s / Tx s

Ty s = 0 \/ Derive Ty s = - Derive Tx s * Tx s / Ty s

*)
  Admitted.

  Lemma is_derive_Tx_abort2 : forall s,
      0 < Tx s ->
      let g := fun v : R => atan2 (Ty v) (Tx v) in
      is_derive Tx s (scal 1 (- sin (g s))).
  Proof.
    intros.
    rewrite Tx_def.
    change (is_derive (fun v : R_AbsRing => cos (g v)) s
                      (scal 1 (- sin (g s)))).
    apply (is_derive_comp cos g s (- sin (g s))).
    auto_derive;[ constructor| field].
    set (f := fun v => atan ((Ty v)/(Tx v))).
    assert (locally s (fun t => f t = g t)) as feqg. {
      unfold locally, f, g.
      simpl.
      admit. }
    simpl in feqg.
...
Derive Tx s =
  (Derive Ty s * Tx s * / (Tx s * Tx s) - Ty s * (Derive Tx s) * / (Tx s * Tx s)) *
 / (1 + Ty s * / Tx s * (Ty s * / Tx s))

Derive Tx s = Derive Ty s * Tx s - Ty s * (Derive Tx s)

Derive Tx s * (1 + Ty s) = Derive Ty s * Tx s 

*)
  Admitted.
  
      
  Lemma is_derive_Ty : forall s, is_derive Ty s (Derive Ty s).
  Proof.
    intros.
  ...
  Lemma is_derive_Tx : forall s, is_derive Tx s (Derive Tx s).
  Proof.
    intros.
    apply Derive_correct.
    apply ex_derive_Tx.
  ...
  Lemma vad : forall s, exists c, c=1\/c=-1 ->
                                  (Nx s, Ny s) = (-c*Ty s, c*Tx s).
  Proof.
    intros.
    unfold Nx, Ny, κ, Tx, Ty.
(*     assert (is_derive Ty  *)
(*     unfold Nx, Ny, κ. *)
(*     Search atan2. *)
(*     set (φ := atan2 (Ty s) (Tx s)). *)
(*     specialize (unitvelocity s) as uv. *)
(*     assert ((Tx s)² + (Ty s)² <> 0) as n0. { *)
(*       intro e0. *)
(*       rewrite e0 in uv. *)
...
(*     apply nzm in n0. *)
(*     specialize (atan2_bound _ _ n0) as pr. *)
(*     change (- PI < φ <= PI) in pr. *)
(*     assert (0 < 1). lra. *)
(*     specialize (atan2_left_inv _ 1 pr ltac:(lra)) as a2a2. *)
(*     unfold φ in a2a2 at 3. *)
(*     unfold atan2 in a2a2. *)
(*     destruct pre_atan2 as [a [abnd [ayd axd]]]. *)
(*     destruct pre_atan2 as [b [bbnd [byd bxd]]]. *)
(*     rewrite Rplus_comm in uv. *)
...
(*     unfold Nx, Ny. *)
(*     assert (is_derive (fun s : R => sin (atan2 (Ty s) (Tx s))) *)
(*                       s (scal 1 (cos φ))) as id. *)
(*     apply (is_derive_comp sin (fun u => atan2 (Ty u) (Tx u))). *)
(*     change (is_derive sin φ (cos φ)). *)
(*     auto_derive. *)
(*     constructor. *)
(*     lra. *)
(*     admit. *)
(*     unfold φ in ayd. *)
(*     Search is_derive. *)
(*     apply is_derive_unique in id. *)
(*     unfold Nx, Ny, κ. *)
(*     assert (is_derive Tx s (- sin φ + cos )). *)
(*     unfold Tx. *)
(*     destruct atan2. *)
(*     specialize (atan2  *)
(*     unfold Nx, Ny, κ, Ty, Tx. *)

(* (/ sqrt ((Derive (Derive Fx) s)² + (Derive (Derive Fy) s)²) * *)
(*    Derive (Derive Fx) s, *)
(*   / sqrt ((Derive (Derive Fx) s)² + (Derive (Derive Fy) s)²) * *)
(*   Derive (Derive Fy) s) = (- Derive Fy s, Derive Fx s) *)

(* (Derive (Derive Fx) s, Derive (Derive Fy) s) = *)
(* sqrt ((Derive (Derive Fx) s)² + (Derive (Derive Fy) s)²) * *)
(* (- Derive Fy s, Derive Fx s)                             *)
    Admitted.
  ...
  Lemma TxNx_def: forall s, Derive Tx s = κ s * Nx s.
  Proof.
    intros.
    unfold Nx.
    field.
  ...
  Lemma TyNy_def: forall s, Derive Ty s = κ s * Ny s.
  Proof.
    intros.
    unfold Ny.
    field.
  ...
  (* change in the direction of acceleration is finite (exists) *)
  Axiom is_derive_Nx : forall s, is_derive Nx s (Derive Nx s).
  Axiom is_derive_Ny : forall s, is_derive Ny s (Derive Ny s).
  ...
  Lemma dTperpT : forall s,
      dot_prod (Derive Tx s, Derive Ty s) (Tx s, Ty s) = 0.
  Proof.
    intros.
    ...
    specialize (is_derive_Tx s) as DTxd.
    specialize (is_derive_Ty s) as DTyd.
  ...
  Proof.
    ...
    specialize (is_derive_Nx s) as DNxd.
    specialize (is_derive_Ny s) as DNyd.
    assert (forall (p q:R), mult p q = mult q p) as Rmult_comm_mult. {
      unfold mult.
      simpl.
      apply Rmult_comm. }
  ...
  Admitted.
    ...
    Lemma bar : forall s, dot_prod (Derive Tx s, Derive Ty s) (Tx s, Ty s) = 0.
  Proof.
    intros.
    assert (dot_prod (Derive Tx s, Derive Ty s) (Tx s, Ty s) =
            / 2 * (dot_prod (Derive Tx s, Derive Ty s) (Tx s, Ty s) +
...
(* end hide *)
  ...
  Lemma Txdef : forall s, Tx s = cos (1 / 2 * PI * (s / l a)²).
  Proof.
    unfold Tx.
    intros.
    specialize (Fx_deriv _ zlta s) as fxd.
  ...
  Lemma Tydef : forall s, Ty s = sin (1 / 2 * PI * (s / l a)²).
  Proof.
    unfold Ty.
    intros.
    specialize (Fy_deriv _ zlta s) as fyd.
